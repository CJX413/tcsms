...
public interface DeviceRegistryDao extends JpaRepository<DeviceRegistry, String>, JpaSpecificationExecutor<DeviceRegistry>, Serializable {
    ...
    @Modifying
    @Transactional
    @Query("update DeviceRegistry dr set dr.isRegistered=:#{#deviceRegistry.isRegistered} where dr.deviceId = :#{#deviceRegistry.deviceId}")
    int deviceRegister(@Param("deviceRegistry") DeviceRegistry deviceRegistry);
}
